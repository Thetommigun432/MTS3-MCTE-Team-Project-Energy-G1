...
export interface UseModelsResult {
  ...
  registerModel: (orgApplianceId: string, name: string, architecture?: string) => Promise<string | null>;
  uploadModelVersion: (modelId: string, version: string, modelFile: File, scalerFile?: File) => Promise<boolean>;
...
export function useModels(buildingId?: string | null): UseModelsResult {
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      if (buildingId) {
        ...
        const enabledApplianceIds = new Set(buildingAppliances?.map(ba => ba.org_appliance_id) || []);
        filteredModels = filteredModels.filter(m => enabledApplianceIds.has(m.org_appliance_id));
      }
  ...
  const registerModel = useCallback(async (
    ...
    try {
      ...
      if (error || !data?.success) {
        toast.error(error?.message || data?.message || 'Failed to register model');
        return null;
      }
  ...
  const uploadModelVersion = useCallback(async (
    modelId: string,
    version: string,
    modelFile: File,
    scalerFile?: File
    ...
    try {
      ...
      const { data: uploadData, error: uploadError } = await edgeFunctions.createModelVersionUpload({
        model_id: modelId,
        version,
      ...
      if (uploadError || !uploadData?.success) {
        toast.error(uploadError?.message || uploadData?.message || 'Failed to create upload');
        return false;
      }
      ...
      const { error: finalizeError } = await edgeFunctions.finalizeModelVersion({
  ...
  const setActiveVersion = useCallback(async (versionId: string): Promise<boolean> => {
    try {
