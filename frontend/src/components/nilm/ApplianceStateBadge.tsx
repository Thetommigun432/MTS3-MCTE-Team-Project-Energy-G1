    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {on ? (
              <Power className={cn('shrink-0', size === 'sm' ? 'h-2.5 w-2.5' : 'h-3 w-3')} />
            ) : (
              <PowerOff className={cn('shrink-0', size === 'sm' ? 'h-2.5 w-2.5' : 'h-3 w-3')} />
            )}
            <span>{on ? 'ON' : 'OFF'}</span>
            {showConfidence && (
              <span className={cn('opacity-70', confidenceColors[confidenceLevel])}>
                {confidencePercent}%
              </span>
            )}
          </Badge>
        ...
        <TooltipContent side="top" className="max-w-xs">
          <div className="space-y-1 text-xs">
            <p className="font-medium">
              {on ? 'Predicted ON' : 'Predicted OFF'} • {confidencePercent}% confidence
            </p>
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      <div className={cn(
        ...
        <div
          className={cn('absolute inset-y-0 left-0 rounded-full transition-all', config.color)}
          style={{ width: `${percent}%` }}
        />
      </div>
