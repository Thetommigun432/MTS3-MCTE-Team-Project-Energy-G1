...
import { Zap, TrendingUp, Cpu, Activity, Download, AlertCircle, ChevronRight, RefreshCw, FileSpreadsheet } from 'lucide-react';
import { LineChart, Line, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import { format, formatDistanceToNow } from 'date-fns';
import { ON_THRESHOLD } from '@/hooks/useNilmCsvData';
...
export default function Dashboard() {
  ...
  const exportToCSV = useCallback(() => {
    ...
    // Build CSV header
    const headers = ['Timestamp', 'Aggregate (kW)', ...appliances.map(a => `${a} (kW)`)];

    // Build CSV rows
    const csvRows = filteredRows.map(row => {
      const applianceValues = appliances.map(a => (row.appliances[a] || 0).toFixed(4));
      return [
  ...
  const exportSummary = useCallback(() => {
    ...
    const summaryLines = [
      ...
      ...topAppliances.map((a, i) => `${i + 1}. ${a.name.replace(/_/g, ' ')}: ${a.totalKwh.toFixed(2)} kWh`),
      '',
      'Current Appliance Status:',
      ...currentApplianceStatus.slice(0, 5).map(a =>
  ...
  }, [filteredRows, dateRange, mode, insights, topAppliances, currentApplianceStatus]);
        ...
        <Card className="border-border overflow-hidden">
          ...
          <CardContent className="pt-2 pb-4">
            <div className="space-y-0">
              ...
              {/* Table rows */}
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center py-3 border-b border-border last:border-0 gap-4">
                  <Skeleton className="h-4 w-28" />
  ...
  if (filteredRows.length === 0) {
    return (
      <div className="space-y-8">
        <DashboardHeader mode={mode} isRefreshing={isRefreshing} lastRefreshed={lastRefreshed} onRefresh={refresh} confidenceLevel={insights.overallConfidence.level} />
        <NILMEmptyState
          title="No data in this range"
          description="Try selecting a wider date range or check your data source connection."
          action={
            <Button variant="outline" size="sm" onClick={() => toast.info('Adjust your date range in the top bar')}>
              Adjust Date Range
            </Button>
          }
          className="min-h-[400px]"
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {mode === 'api' && (
        <div className={`rounded-lg px-4 py-3 text-sm text-foreground flex items-center gap-2 ${
          error || !filteredRows.length 
            ? 'bg-destructive/10 border border-destructive/30' 
            : 'bg-energy-warning-bg/50 border border-energy-warning/30'
        }`}>
          <AlertCircle className={`h-4 w-4 shrink-0 ${error || !filteredRows.length ? 'text-destructive' : 'text-energy-warning'}`} />
          <span>
            <span className={`font-medium ${error || !filteredRows.length ? 'text-destructive' : 'text-energy-warning'}`}>API mode</span>
            {' — '}
            {error ? (
              <span>{error}</span>
            ) : !filteredRows.length ? (
              <span>No data available. Click <strong>Refresh</strong> to fetch from API.</span>
            ) : (
        ...
        <Card className="border-border">
          <CardContent className="pt-5 pb-4">
            <MetricCardContent
              ...
              subtitle={insights.peakLoad.timestamp ? format(new Date(insights.peakLoad.timestamp), 'MM/dd HH:mm') : '—'}
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <NILMPanel
          ...
          footer={mode === 'demo' ? 'Ground truth from sub-meters (training data)' : 'Data from smart meter • Estimated by AI'}
        >
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [`${value.toFixed(3)} kW`, 'Total']}
                />
                <Line
                  type="monotone"
                  dataKey="total"
        ...
        <NILMPanel
          ...
          footer={mode === 'demo' ? 'Sub-metered readings (not AI predictions)' : 'AI-predicted breakdown • Colorblind-safe palette'}
        >
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                ...
                <Legend
                  iconType="circle"
                  iconSize={8}
                  wrapperStyle={{ fontSize: 10, paddingTop: 8, color: 'hsl(var(--muted-foreground))' }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-3 font-medium text-muted-foreground">Appliance</th>
                <th className="text-left py-3 font-medium text-muted-foreground">State</th>
                <th className="text-left py-3 font-medium text-muted-foreground">{mode === 'demo' ? 'Data Quality' : 'Confidence'}</th>
                <th className="text-right py-3 font-medium text-muted-foreground">{mode === 'demo' ? 'Actual kW' : 'Est. kW'}</th>
                {managedAppliances.length > 0 && (
                  <th className="text-right py-3 font-medium text-muted-foreground hidden sm:table-cell">Rated kW</th>
                )}
              </tr>
            </thead>
            <tbody>
              {currentApplianceStatus.map((a) => (
                <tr
                  key={a.name}
                ...
                >
                  ...
                  <td className="py-3">
                    <ApplianceStateBadge on={a.on} confidence={a.confidence} size="sm" />
                  </td>
                  <td className="py-3">
                    <ConfidenceIndicator confidence={a.confidence} size="sm" />
                  </td>
                  <td className="py-3 text-right">
                    <span className="metric-value text-foreground">{a.est_kW.toFixed(3)}</span>
                  </td>
                  {managedAppliances.length > 0 && (
                    <td className="py-3 text-right hidden sm:table-cell">
                      {a.rated_kW ? (
                        <span className="text-muted-foreground">{a.rated_kW} kW</span>
                      ) : (
      ...
      <ApplianceDetailModal
        ...
        currentStatus={currentApplianceStatus.find((a) => a.name === selectedAppliance)}
      />
    </div>
  );
}
...
function DashboardHeader({ mode, isRefreshing, lastRefreshed, onRefresh, confidenceLevel }: DashboardHeaderProps) {
  return (
    <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div className="space-y-1 relative">
        ...
        <p className="text-sm text-muted-foreground">
          {mode === 'demo'
            ...
            : 'AI-powered energy disaggregation — predicting appliance states from total consumption'
          }
        </p>
      </div>
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onRefresh}
            disabled={isRefreshing}
            aria-label={isRefreshing ? 'Refreshing data...' : 'Refresh energy data'}
