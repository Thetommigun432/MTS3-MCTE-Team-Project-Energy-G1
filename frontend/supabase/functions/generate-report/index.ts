...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!building_id || !start_date || !end_date) {
      return new Response(
        JSON.stringify({ error: 'building_id, start_date, and end_date are required' }),
    ...
    const applianceEnergy: Record<string, { name: string; slug: string; totalKwh: number; totalConfidence: number; count: number }> = {};
    
    for (const pred of (predictions || [])) {
      const appliance = (pred as { org_appliances?: { slug?: string; name?: string } }).org_appliances;
      const slug = appliance?.slug || 'unknown';
      const name = appliance?.name || 'Unknown';
      
      if (!applianceEnergy[slug]) {
        applianceEnergy[slug] = { name, slug, totalKwh: 0, totalConfidence: 0, count: 0 };
      }
    ...
    if (readings && readings.length > 0) {
      ...
      totalEnergy = Object.values(applianceEnergy).reduce((sum, a) => sum + a.totalKwh, 0);
    }
    ...
    // Build breakdown
    const totalFromAppliances = Object.values(applianceEnergy).reduce((sum, a) => sum + a.totalKwh, 0);
    const breakdown = Object.values(applianceEnergy)
      .map(a => ({
        appliance_name: a.name,
        appliance_slug: a.slug,
        energy_kwh: a.totalKwh,
        percentage: totalFromAppliances > 0 ? (a.totalKwh / totalFromAppliances) * 100 : 0,
    ...
    console.log(`Report generated for building ${building_id}: ${dataPointsAnalyzed} predictions analyzed`);
