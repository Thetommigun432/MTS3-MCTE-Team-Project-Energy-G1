...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const applianceSeries: Record<string, { ts: string; power_kw: number; confidence: number }[]> = {};
    const latestPredictions: Record<string, { power_kw: number; is_on: boolean; confidence: number; timestamp: string }> = {};

    for (const pred of (predictions || [])) {
      const appliance = (pred as unknown as { org_appliances: { id: string; name: string; slug: string } }).org_appliances;
      ...
      if (!latestPredictions[slug] || pred.timestamp > latestPredictions[slug].timestamp) {
    ...
    if (aggregateSeries.length > 0) {
      const peak = aggregateSeries.reduce((max, r) => r.power_kw > max.power_kw ? r : max, aggregateSeries[0]);
      insights.push({
        type: 'peak_load',
        label: 'Peak Load',
    ...
    const totalEnergy = aggregateSeries.reduce((sum, r) => sum + r.power_kw * 0.25, 0);
    insights.push({
      type: 'daily_usage',
      label: 'Total Energy',
