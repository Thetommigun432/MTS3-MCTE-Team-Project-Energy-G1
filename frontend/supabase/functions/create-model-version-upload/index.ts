...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingVersion) {
      return new Response(
        JSON.stringify({ error: `Version ${version} already exists for this model` }),
    ...
    const scalerPath = has_scaler ? `${user.id}/${model_id}/${version}/scaler.pkl` : null;
    ...
    if (versionError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create model version', details: versionError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Generate signed upload URLs
    const { data: modelUploadData, error: modelUploadError } = await supabaseClient.storage
      .from('models')
    ...
    if (modelUploadError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create upload URL for model artifact' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (has_scaler && scalerPath) {
      const { data: scalerUploadData, error: scalerUploadError } = await supabaseClient.storage
        .from('models')
    ...
    console.log(`Model version ${version} created for model ${model_id} by user ${user.id}`);
