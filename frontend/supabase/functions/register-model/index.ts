...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingModel) {
      return new Response(
        JSON.stringify({ error: 'A model already exists for this appliance', model_id: existingModel.id }),
    ...
    if (createError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create model', details: createError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Model ${model.id} created for appliance ${appliance.name} by user ${user.id}`);
