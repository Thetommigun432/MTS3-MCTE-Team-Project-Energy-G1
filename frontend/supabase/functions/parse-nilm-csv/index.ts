...
function validateAndSanitizeNumber(value: string | undefined, min: number, max: number): number {
...
serve(async (req) => {
  ...
  try {
    ...
    if (contentSize > MAX_FILE_SIZE_BYTES) {
      console.error(`File too large: ${contentSize} bytes exceeds ${MAX_FILE_SIZE_BYTES}`);
      return new Response(
        JSON.stringify({ error: `File size exceeds ${MAX_FILE_SIZE_BYTES / 1024 / 1024}MB limit` }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (rawRows.length > MAX_ROW_COUNT) {
      console.error(`Too many rows: ${rawRows.length} exceeds ${MAX_ROW_COUNT}`);
      return new Response(
    ...
    for (const row of rawRows) {
      ...
      const aggregate = validateAndSanitizeNumber(row['Aggregate'], 0, MAX_KW_VALUE);

      const appliances: Record<string, number> = {};
      for (const col of APPLIANCE_COLUMNS) {
        appliances[col] = validateAndSanitizeNumber(row[col], MIN_KW_VALUE, MAX_KW_VALUE);
    ...
    // Sort by time
    validatedRows.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());

    console.log(`Validated ${validatedRows.length} rows, skipped ${skippedRows} invalid rows`);
