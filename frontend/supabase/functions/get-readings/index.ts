...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (aggregateError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch readings', details: aggregateError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Fetch predictions for the building
    const { data: predictionsData, error: predictionsError } = await supabaseClient
      .from('predictions')
      .select(`
        timestamp,
        power_kw,
    ...
    for (const prediction of (predictionsData || [])) {
      ...
      const slug = (prediction as { org_appliances?: { slug?: string } }).org_appliances?.slug || 'unknown';
    ...
    console.log(`Fetched ${readings.length} readings for building ${building_id}`);
