...
class TrainingConfig:
    ...
    # Data paths
    data_path: Path = Path('data/processed/1sec_new/model_ready')
    save_path: Path = Path('transformer/checkpoints')
    ...
    # Loss: 'mse' (paper), 'sparsity' (improved), 'weighted' (FN penalty)
...
def create_loss(loss_type: str) -> nn.Module:
    ...
    if loss_type == 'mse':
        ...
        print(f"Unknown loss type '{loss_type}', defaulting to sparsity")
        return SparsityAwareLoss()
...
def train(config: TrainingConfig):
    ...
    # Load data
    train_data, val_data, test_data, metadata = load_pretrained_data(config.data_path)
    
    # Get appliances from metadata
    appliances = metadata.get('target_appliances', config.appliances)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss', type=str, default='sparsity', 
                        choices=['mse', 'sparsity', 'weighted'],
                        help='Loss type: mse (paper), sparsity (improved), weighted (FN penalty)')
