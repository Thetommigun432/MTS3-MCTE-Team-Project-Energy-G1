...
References:
- NILMFormer (Petralia et al., KDD 2025) - Non-stationarity handling
- BERT4NILM (Yue et al., 2020)
- ELECTRIcity (Sykiotis et al., 2022)
- GTCN (Zai et al., 2023)
- Energformer (Angelis et al., 2023)
"""
import torch
...
class CausalStationarization(nn.Module):
    ...
    OPTIMIZED: Uses cumsum for O(n) instead of O(n²) loop.
    Normalizes input using only past information (no data leakage).
    
    Note: This is a numerically stable approximation that works well in practice.
    For exact Welford semantics, the loop version would be needed but is slow.
    """
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Cumulative variance approximation using E[X²] - E[X]²
        # This is biased but fast and works well in practice
        x_sq = x ** 2
        cumsum_sq = x_sq.cumsum(dim=1)
        mean_sq = cumsum_sq / counts  # E[X²]
        sq_mean = causal_mean ** 2    # E[X]²
...
class DeStationarization(nn.Module):
    ...
    FIX: Previous version used learned projection which could produce negative values.
    Now we directly use the causal mean/std from stationarization.
    """
    ...
    def forward(self, x: torch.Tensor, causal_mean: torch.Tensor, 
                causal_std: torch.Tensor) -> torch.Tensor:
        """
        ...
        # FIX: Use original stats directly, not learned projection
        # Denormalize: x is in normalized space, scale back to original
...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, in_features: int, channels: List[int] = [32, 64, 128],
                 kernel_sizes: List[int] = [7, 5, 3], use_residual: bool = True):
        super().__init__()
        ...
        for i in range(len(channels) - 1):
            if use_residual:
                layers.append(ResidualConvBlock(channels[i], channels[i+1], kernel_sizes[i]))
            else:
                layers.append(nn.Conv1d(channels[i], channels[i+1], kernel_sizes[i],
...
# =============================================================================
# MULTI-TASK OUTPUT HEAD (Improved: neighborhood regression + pooled classification)
# =============================================================================
class ApplianceHead(nn.Module):
    ...
    - Regression: Uses neighborhood around midpoint (±5 timestep) with attention
      This is more robust for streaming than single midpoint.
    - Classification: Uses temporal pooling (captures context for ON/OFF)
    
    This separation is critical: pooling helps classification but hurts regression.
    """
    
    def __init__(self, d_model: int, seq2point: bool = True, dropout: float = 0.1,
                 use_pooling_for_state: bool = True, neighborhood_size: int = 5):
        super().__init__()
        self.seq2point = seq2point
        self.use_pooling_for_state = use_pooling_for_state
        # For simplified model, keep it simple (0 neighborhood)
        self.neighborhood_size = neighborhood_size
        
        # Output layers directly if simpler head desired, but preserving attention structure is usually better
        # For full compatibility let's keep complex head but maybe allow simpler one?
        # Sticking to valid head logic
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.seq2point:
            if x.dim() == 3:
                ...
                # Should not happen for seq2point typically
                power = self.regression(x)
                state = self.classification(x)
        else:
            # SEQ2SEQ: Predict for every timestep
            # No global pooling allowed, as we need [Batch, SeqLen, 1] output
            power = self.regression(x)
            
            # For classification in Seq2Seq, we can't use global pooling
            # We must use per-timestep features
            if self.use_pooling_for_state:
                 # If model was trained with pooling, we can't easily switch to seq2seq without retraining
                 # BUT for SimpleNILMTransformer, use_pooling_for_state is False by default
                 # We fallback to simple projection if pooling requested in seq2seq mode (or error?)
                 # For now, let's assume we use local features for state in seq2seq
                 state_input = x # [Batch, SeqLen, D_model]
                 # We need to project if classification expected 3x D_model
                 # This is a limitation: Hybrid model expecting pooling can't run Seq2Seq easily
                 pass 
...
class HybridCNNTransformer(nn.Module):
    """
    Complex Hybrid CNN-Transformer (Existing).
    """
    ...
    def __init__(
        ...
        cnn_kernel_sizes: List[int] = [7, 5, 3],
        use_rope: bool = True,
        seq2point: bool = True,
        max_len: int = 2048,
...
# =============================================================================
# SIMPLE NILM TRANSFORMER (Notebook-aligned)
# =============================================================================
class SimpleNILMTransformer(nn.Module):
    """
    Simplified CNN-Transformer that matches the high-performing notebook architecture.
    
    Architecture:
    1. Single Conv1d layer (no complex residuals)
    2. BatchNorm + ReLU + Dropout
    3. Positional Encoding
    4. Standard Transformer Encoder
    5. Appliance Head
    
    This simpler architecture often generalizes better on smaller datasets.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.appliances = appliances or ['heatpump']
        
        # 1. Simple embedding: Conv1D k=5 -> d_model
        # Matches notebook: self.embedding = nn.Conv1d(in_c, d_model, kernel_size=5, padding=2)
        ...
        # 2. Positional Encoding (Standard sinusoid)
        self.pos_encoder = SinusoidalPositionalEncoding(d_model, max_len, dropout)
        
        # 3. Transformer Encoder (Standard PyTorch implementation)
        # Matches notebook layer config
        ...
        # 4. Heads (Simplified)
        # Notebook used [d_model -> d_model//2 -> 1]
        self.heads = nn.ModuleDict({
            name: ApplianceHead(
                d_model, 
                seq2point=seq2point, 
                dropout=dropout, 
                use_pooling_for_state=False, # Simpler head
                neighborhood_size=0 # Strict midpoint regression
            )
            for name in self.appliances
        })
