"""
TCN_SA - TCN + Self-Attention Architecture (SOTA Real-Time NILM)
=================================================================
...
class TCN_SA(nn.Module):
    """
    TCN_SA - SOTA Causal Architecture for Real-Time NILM
    ...
    def __init__(self,
                 ...
                 window: int = 4096,  # Required window size (minimum 4096)
                 inception_causal: bool = True):  # False for EVCharger (trained with lookahead)
        super().__init__()
        
        self.window = window  # Store for validation
        
        self.n_appliances = n_appliances
        self.use_psa = use_psa
        self.use_inception = use_inception
        self.lookahead = lookahead
        self.inception_causal = inception_causal  # Store for inception block
        ...
        if use_inception:
            # Use inception_causal parameter (False for EVCharger trained with lookahead)
            self.inception = MultiScaleCausalInception(hidden_channels, hidden_channels, causal=inception_causal)
...
if __name__ == "__main__":
    ...
    print(f"TCN_SA Parameters: {count_parameters(model):,}")
    ...
    configs = [
        ("Full (Inception + Attention)", True, True),
    ...
    for name, use_inception, use_psa in configs:
        m = TCN_SA(use_inception=use_inception, use_psa=use_psa)
