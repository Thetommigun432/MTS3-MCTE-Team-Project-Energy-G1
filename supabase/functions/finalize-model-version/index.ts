...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (version.status !== "uploading") {
      return new Response(
        JSON.stringify({
          error: `Cannot finalize version in status: ${version.status}`,
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: fileData, error: _fileError } = await supabaseClient.storage
      .from("models")
    ...
    if (!hasModelFile) {
      ...
      return new Response(
        JSON.stringify({
          error: "Model artifact not found. Upload may have failed.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
