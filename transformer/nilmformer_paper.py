...
class DiagonallyMaskedSelfAttention(nn.Module):
    ...
    Uses PyTorch's scaled_dot_product_attention for FlashAttention support
    on H200/H100 GPUs. This reduces memory from O(L^2) to O(L) and is 3-5x faster.
    ...
    def __init__(
        ...
        # Cache for diagonal mask (created once per sequence length)
        self._cached_mask = None
        self._cached_seqlen = 0

    def _get_diagonal_mask(self, seqlen: int, device: torch.device) -> torch.Tensor:
        """Get or create diagonal attention mask (True = masked out)."""
        if self._cached_seqlen != seqlen or self._cached_mask is None:
            # Create mask: True on diagonal positions (to be masked to -inf)
            self._cached_mask = torch.diag(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Manual attention computation (more compatible than scaled_dot_product_attention)
        scale = 1.0 / (self.head_dim ** 0.5)
        scores = torch.matmul(xq, xk.transpose(-2, -1)) * scale  # (B, H, L, L)
        
        # Apply diagonal mask (mask out self-attention)
        diag_mask = self._get_diagonal_mask(seqlen, x.device)  # (L, L)
...
class NILMFormerPaper(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Input: (B, L, C) where C = 1 + c_embedding
            - Channel 0: aggregate power (load curve)
            - Channels 1+: exogenous inputs (temporal sin/cos)
        
        Output: (B, L, 1) predicted appliance power
        """
        ...
        # Permute to (B, L, 1) for consistency with dataset format
        x = x.permute(0, 2, 1)
...
class MultiApplianceNILMFormer(nn.Module):
    ...
    def __init__(
        ...
        # NO de-normalization: targets are already in [0, 1] normalized space
        self.heads = nn.ModuleDict()
        
        for app in appliances:
            # Output head: Conv -> Sigmoid to bound output in [0, 1]
            self.heads[app] = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        # Per-appliance heads (NO de-normalization, direct prediction)
        outputs = {}
        for app in self.appliances:
            # Conv head -> direct output
            y = self.heads[app](x_seq)  # (B, 1, L)
            
            # ReLU to ensure non-negative (power can't be negative)
            y = F.relu(y)
...
def create_nilmformer_paper(
    ...
    if multi_appliance and len(appliances) > 1:
        ...
        # Single appliance - wrap in a class that returns dict output
        config = NILMFormerConfig(
            c_in=1,
            c_embedding=c_embedding,
            c_out=1,
        ...
        class SingleApplianceWrapper(nn.Module):
            """Wrapper to make NILMFormerPaper return dict like MultiApplianceNILMFormer."""
            def __init__(self, model, appliance_name):
                super().__init__()
                self.model = model
                self.appliance_name = appliance_name
            
            def forward(self, x):
                out = self.model(x)  # (B, L, 1)
                return {self.appliance_name: out}
        
        return SingleApplianceWrapper(NILMFormerPaper(config), appliances[0])
